...
interface ActivityRowProps {
  ...
  entry: {
    ...
    activity: { name: string; category: string | null; durationMin: number | null; estimatedCost: number | null } | null;
  };
  currencyCode: string;
}
  ...
  return (
    <div className="activity-row">
      <div className="activity-row-info">
        <span className="activity-category-dot"
          data-category={entry.activity?.category ?? "other"} />
...
interface Props {
  tripId: string;
  stop: Stop & { city: { name: string; countryCode: string | null } };
  currencyCode: string;
}
...
export function SortableStop({ tripId, stop, currencyCode }: Props) {
  ...
  const loadActivities = async () => {
    ...
    const city = result[0];
    if (city) {
      const acts = await fetch(`/api/v1/explore/cities/${city.id}/activities`)
        .then((r) => r.json())
    ...
    <div ref={setNodeRef} style={style} className={`builder-stop ${isDragging ? "builder-stop--dragging" : ""}`}>
      ...
      <div className="builder-stop-header">
        ...
        <div className="builder-stop-actions">
          <span className="stop-activity-count">{activities.length} activities</span>
          <button
      ...
      {expanded && (
        <div className="builder-stop-body">
          {activities.length === 0 ? (
            <p className="no-activities">No activities yet — add some below 🎟️</p>
          ) : (
          ...
          {showAddActivity && (
            <div className="add-activity-panel">
              <div className="add-activity-header">
                ...
                <button className="btn-ghost-sm" onClick={() => setShowAddActivity(false)}>Close</button>
              </div>
              <input
                type="text"
              ...
              />
              {filtered.length === 0 && <p className="field-hint">No activities found for this city in the seed data.</p>}
                  ...
                  <li key={act.id} className="activity-search-item">
                    ...
                    <div className="activity-search-right">
                      {act.estimatedCost !== null && (
                        <span className="activity-price">${act.estimatedCost}</span>
                      )}
                      <button
                        className="btn btn-primary btn-sm"
                        onClick={() => {
                          addActivity.mutate({ activityId: act.id, cost: act.estimatedCost ?? undefined });
