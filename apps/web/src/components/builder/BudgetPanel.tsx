...
function ProgressBar({ value, max, danger }: { value: number; max: number; danger: boolean }) {
  ...
  return (
    <div className="budget-progress-bar">
      <div
        className={`budget-progress-fill ${danger ? "budget-progress--danger" : ""}`}
        style={{ width: `${pct}%` }}
      />
    </div>
  );
...
export function BudgetPanel({ summary }: Props) {
  ...
  return (
    <aside className="budget-panel">
      ...
      <div className={`budget-total-card ${isOverBudget ? "budget-total--over" : ""}`}>
        ...
        {budgetLimit !== null && (
          <div className="budget-limit-block">
            <p className="budget-limit-label">of {fmt(budgetLimit)} budget</p>
            {remaining !== null && (
              <p className={`budget-remaining ${isOverBudget ? "text-danger" : "text-success"}`}>
                {isOverBudget ? `⚠️ ${fmt(Math.abs(remaining))} over` : `✅ ${fmt(remaining)} left`}
              </p>
            )}
          </div>
        )}
      ...
      {/* Progress bar */}
      {budgetLimit !== null && (
        <ProgressBar value={grandTotal} max={budgetLimit} danger={isOverBudget} />
      )}
      ...
      <div className="budget-categories">
        {(Object.entries(breakdown) as [string, number][]).map(([key, val]) => (
          <div key={key} className="budget-cat-row">
            ...
            <span className="budget-cat-label">{key}</span>
            ...
            {stops.map((s) => (
              <div key={s.stopId} className="budget-stop-row">
                ...
                <div className="budget-stop-costs">
                  {s.accommodation > 0 && <span>🏨 {fmt(s.accommodation)}</span>}
                  {s.transport > 0 && <span>🚆 {fmt(s.transport)}</span>}
                  {s.meals > 0 && <span>🍽️ {fmt(s.meals)}</span>}
                  {s.activities > 0 && <span>🎟️ {fmt(s.activities)}</span>}
                </div>
